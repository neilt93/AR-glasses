...
Assistant keys: `q` quit, `h` labels, `c` crosshair, `i` info, `k` hands, `d` depth overlay.
...
| `src/perception/depth.py` | Monocular depth estimation (MiDaS): spatial reasoning, object depth sorting |
| `src/perception/detector.py` | Assembly-specific YOLO detector (mock fallback) |
| `src/hud/widgets.py` | Composable widget HUD: ObjectLabels, TrackedObjectLabels, HandSkeleton, DepthOverlay, StatusBar, Notifications, InfoPanel, Crosshair |
...
Core: `opencv-python`, `numpy`, `pyyaml`, `ultralytics` (YOLOv8). Optional: `pyttsx3` (voice), `mediapipe` (hands), `torch` (depth/MiDaS), `pytesseract` (OCR), `scikit-learn` (failure classifier).
